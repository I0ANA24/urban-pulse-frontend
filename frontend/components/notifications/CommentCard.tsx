...
export default function CommentCard({ n, onRead }: { n: NotificationItem; onRead: () => void }) {
  ...
  return (
    <div onClick={onRead} className="cursor-pointer active:scale-95 transition-all duration-200 relative pt-7">
      ...
      >
        <div className="px-4 pt-3 pb-4" style={{ background: "#1C2229" }}>

          {/* Time + unread dot row */}
          <div className="flex items-center justify-end gap-1.5 mb-2">
            {!n.isRead && <span className="w-2 h-2 rounded-full bg-[#BEDCF5] shrink-0" />}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex flex-col gap-1 flex-1 min-w-0">
              <p className={`text-sm font-bold truncate ${n.isRead ? "text-white/40" : "text-white"}`}>
                {n.title}
              </p>
              <p className={`text-sm leading-snug ${n.isRead ? "text-white/30" : "text-white/70"}`}>
                {n.body}
              </p>
            </div>
          </div>
